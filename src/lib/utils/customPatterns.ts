...
export type ResolvedFurnitureDef = CustomPattern & { category: string; color: string; height: number; icon: string; pattern: true };

function normalizePattern(pattern: CustomPattern): CustomPattern {
  ...
  return { ...pattern, id: pattern.id.trim(), name: pattern.name.trim(), category: pattern.category?.trim() || '自定义物件', shape: pattern.shape ?? 'rectangle' };
}
...
export function getResolvedFurniture(id: string): ResolvedFurnitureDef | undefined {
  ...
  return { ...pattern, category: pattern.category || '自定义图案', color: pattern.color || '#64748b', height: pattern.height ?? 0, icon: '图', pattern: true };
}
